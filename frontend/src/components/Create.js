...
//   const [permission, setPermission] = useState('read');
//   const [isDropdownOpen, setIsDropdownOpen] = useState(false);
//   const { data, error } = useFetch(`${process.env.REACT_APP_API_URL}/user/`);
//   const [authors, setAuthors] = useState(JSON.parse(localStorage.getItem(`${docId}`)) || []);
//   const socket = useRef();
//   const { user : currUser } = useAuthStore() ; 
//   const currUserMail = currUser.email ;
//   const currentUser = { email: currUserMail }; // Replace with actual current user logic

//   useEffect(() => {
//     socket.current = io(process.env.REACT_APP_API_URL2);
//     socket.current.emit('join-doc', docId);

//     socket.current.on('receive-changes', ({ field, value }) => {
//       if (field === 'content') {
//         setContent(value);
//       } else if (field === 'title') {
//         setTitle(value);
...
//     setTitle(newTitle);
//     socket.current.emit('send-changes', { docId, field: 'title', value: newTitle });
//   };

//   const handleContentChange = (value) => {
//     setContent(value);
//     socket.current.emit('send-changes', { docId, field: 'body', value });
//   };

//   const handleSave = async () => {
//     try {
//       const res = await fetch(`${process.env.REACT_APP_API_URL}/docs/body/${docId}`, {
//         method: 'PATCH',
//         headers: {
//           'Content-Type': 'application/json',
//         },
...
//     }
//     // socket.current.emit('save-doc', docId, {title : }); // Save document
//     // console.log('Document saved');
//   };
...
//     if (selectedUser) {
//       const user = data.find((user) => user._id === selectedUser);
//       if (user && !authors.find((author) => author.userMail === user.email)) {
//         const userMail = user.email;
//         setAuthors([...authors, { userMail, permission }]);
//         try {
//           const res = await fetch(`${process.env.REACT_APP_API_URL}/docs/${docId}`, {
//             method: 'PATCH',
//             headers: {
//               'Content-Type': 'application/json',
//             },
//             body: JSON.stringify({ userMail, permission }),
//           });
//           if (res.ok) {
//             localStorage.setItem(`${docId}`, JSON.stringify([...authors, { userMail, permission }]));
//           }
//         } catch (error) {
//           console.log(error);
//         }
...
//   const handleRemoveAuthor = async (userMail) => {
//     const updatedAuthors = authors.filter((author) => author.userMail !== userMail);
//     setAuthors(updatedAuthors);
//     try {
//       const res = await fetch(`${process.env.REACT_APP_API_URL}/docs/authors/${docId}`, {
//         method: 'PATCH',
//         headers: {
//           'Content-Type': 'application/json',
//         },
...
//       });
//       if (res.ok) {
//         localStorage.setItem(`${docId}`, JSON.stringify(updatedAuthors));
//       }
//     } catch (error) {
//       console.log(error);
//     }
...
//   const userPermission = authors.find((author) => author.userMail === currentUser.email) ?.permission || 'read';
//   const isReadOnly = userPermission === 'read';
//   const isOwner = (currentUser.email == authors[0].userMail)
//   return (
//     <div className="min-h-screen bg-gray-200 flex flex-col items-center">
//       <div className="bg-white shadow p-4 fixed w-full top-0 z-10">
//         <div className="container mx-auto flex justify-between items-center">
...
//                       <div key={userMail} className="flex justify-between items-center p-2 border-b border-gray-10">
//                         <span>
//                           {userMail} - {permission == null ? 'owner' : permission}
//                         </span>
//                         {permission && (
//                           <button
//                             onClick={() => handleRemoveAuthor(userMail)}
